...
public class H {
    ...
    public static class ImpTreap {
        ...
        public Node del(long key, Node x) {
            Pair dop = split(x, key);
            return merge(dop.getFirst(), split(dop.getSecond(), 1).getSecond());
        }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            impT.peak = impT.add(i, in.nextInt(), impT.peak);
        }

        for (int i = 0; i < m; i++) {
            String mode = in.next();

            if (Objects.equals(mode, "del")) {
                impT.peak = impT.del(in.nextInt() - 1, impT.peak);
            } else if (Objects.equals(mode, "add")) {
                impT.peak = impT.add(in.nextInt() , in.nextInt(), impT.peak);
            }
        }

        System.out.println(impT.getCount(impT.peak));
        impT.BST(impT.peak);
